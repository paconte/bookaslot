<script>
	...
	const addBooking = (item) => {
		...
		item.humanDate = formatDate(item.start * 1000);
		item.humanStart = formatTime(item.start * 1000);
		item.humanFinish = formatTime(item.finish * 1000);
			...
			<tbody class="bg-white">
				{#each currentData as row}
					<tr class="text-gray-700">
						{#each row as cell}
							<td
								on:click={() => bookItem(cell)}
								class="px-4 py-3 border {getCellBackgroundColor(
									cell
								)}"
							>
...
{#if bookingsLength > 0}
<div class="w-full lg:w-5/6 mb-8 overflow-hidden rounded-lg shadow-lg">
	<div class="w-full overflow-x-auto">
		<table class="w-full">
			<thead>
				<tr
			...
			<tbody class="bg-white">
				{#each Array.from($bookings) as item}
					<tr class="text-gray-700 border-b">
						<td class="px-4 py-3">{item.humanDate}</td>
						<td class="px-4 py-3">{item.humanStart}</td>
						<td class="px-4 py-3">{item.humanFinish}</td>
	...
	<div
		class="block bg-gray-200 text-sm text-right py-4 px-4 -mx-3 -mb-2 rounded-b-lg"
	>
		<button
			on:click={bookSlots}
			type="button"
